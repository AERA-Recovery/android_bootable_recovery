...
```sh
python3 aeraui/i18n/generate_catalog.py \
  --recovery . \
  --output aeraui/i18n/catalog_generated.cpp
```

The generator emits only catalog entries that are still referenced by native
AERA UI source. The legacy XML theme tree is no longer packaged; translations
