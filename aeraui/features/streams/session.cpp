...
void Session::Close() {
  ...
  frame_width_ = kLandscapeWidth;
  frame_height_ = kLandscapeHeight;
  playing_ = false;
}
...
bool Session::Adopt(int frame_fd, int control_fd) {
  ...
  const bool valid =
      frame_fd >= 0 && control_fd >= 0 && frame_fd != control_fd && !fstat(frame_fd, &info) &&
  ...
  if (!valid) {
    ...
    status_ = "Invalid AERA Streams channel.";
    return false;
  }
  void* mapped = mmap(nullptr, kSharedBytes, PROT_READ, MAP_SHARED, frame_fd, 0);
  close(frame_fd);
  if (mapped == MAP_FAILED) {
    close(control_fd);
    status_ = "Could not map stream video frames.";
    return false;
  }
  shared_ = static_cast<uint8_t*>(mapped);
  control_ = control_fd;
  fcntl(control_, F_SETFL, fcntl(control_, F_GETFL) | O_NONBLOCK);
  status_ = "Stream player ready";
  return true;
}
...
bool Session::Write(const Message& message) {
  ...
  if (send(control_, &message, sizeof(message), MSG_DONTWAIT | MSG_NOSIGNAL) ==
    ...
    return true;
  Fail("AERA Streams stopped responding.");
  return false;
}

bool Session::Send(Kind kind, const char* text, int64_t value) {
  if (kind == Kind::kOpen) {
  ...
  Message message;
  message.kind = kind;
  if (kind == Kind::kSeekAbsolute) message.position_ms = value;
...
bool Session::Poll() {
  ...
  for (int i = 0; i < 12 && Connected(); ++i) {
    ...
    if (count != static_cast<ssize_t>(sizeof(message)) || !Valid(message, true)) {
      Fail("Stream player closed or sent an invalid response.");
      break;
    }
    if (message.kind == Kind::kFrame) {
      if (frame_pending_ || message.sequence != sequence_ + 1) {
        Fail("Stream frame sequence mismatch.");
        break;
      }
      sequence_ = message.sequence;
      frame_width_ = message.x;
      frame_height_ = message.y;
      frame_pending_ = true;
      changed = true;
    } else {
