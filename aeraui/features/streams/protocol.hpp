...
constexpr uint32_t kMagic = 0x41535431U;
...
constexpr uint32_t FrameBytes(int width, int height) {
  return width > 0 && height > 0 ? static_cast<uint32_t>(width) * static_cast<uint32_t>(height) * 4U
                                 : 0;
}
constexpr bool ValidFrame(int width, int height, uint32_t bytes) {
  return width > 0 && height > 0 && width <= kMaxDimension && height <= kMaxDimension &&
         static_cast<uint32_t>(width) * static_cast<uint32_t>(height) <= kMaxPixels &&
...
static_assert(sizeof(Message) == 4136);

inline bool Valid(const Message& message, bool from_worker) {
  if (message.magic != kMagic || !memchr(message.text, '\0', sizeof(message.text))) return false;
  if (from_worker)
    return ((message.kind == Kind::kFrame || message.kind == Kind::kThumbnailFrame) &&
  ...
  return message.kind == Kind::kOpen || message.kind == Kind::kPlay ||
         message.kind == Kind::kPause || message.kind == Kind::kSeekAbsolute ||
