...
// AERA RPC progress is emitted directly as structured JSON. It no longer uses
// sentinel text inside the GUI log stream, so clients never have to scrape or
// strip recovery output.
extern "C" void gui_aera_progress_overall(const int percent)
{
	aera::rpc::Dispatcher::Progress("overall", percent);
}

extern "C" void gui_aera_progress_item(const int percent)
{
	aera::rpc::Dispatcher::Progress("item", percent);
}

extern "C" void gui_aera_progress_detail(const char* phase, const int percent, const char* label,
