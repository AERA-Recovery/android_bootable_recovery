...
bool Process::Start(const std::string &runtime, int &frame_fd, int &control_fd,
  ...
  if (pid_ >= 0 || runtime.size() != 22 ||
      runtime.compare(0, 16, "/tmp/aera-media-")) {
    error = "The verified media runtime path is invalid."; return false;
  }
  struct stat root{};
  if (lstat(runtime.c_str(), &root) || !S_ISDIR(root.st_mode) || root.st_uid ||
      (root.st_mode & 0777) != 0700) {
    error = "The media runtime is no longer private."; return false;
  }
  int frame = memfd_create("aera-media-pixels", MFD_CLOEXEC | MFD_ALLOW_SEALING);
  int channels[2] = {-1, -1};
  ...
  if (child_frame < 0 || child_control < 0) {
    ...
    close(frame); close(channels[0]); close(channels[1]); StopAudio();
    error = "Could not protect media descriptors."; return false;
  }
  const pid_t child = fork();
  if (!child) {
    if (dup2(child_frame, 3) < 0 || dup2(child_control, 4) < 0) _exit(78);
    close(child_frame); close(child_control);
    execl("/system/bin/aera-browser-jail", "aera-browser-jail", "--media",
          runtime.c_str(), nullptr);
    _exit(78);
  }
  close(child_frame); close(child_control); close(channels[1]);
  if (child < 0) {
    close(frame); close(channels[0]); StopAudio();
    error = "Could not start the isolated media supervisor."; return false;
  }
...
bool Process::Running() {
  ...
  int status = 0; const pid_t result = waitpid(pid_, &status, WNOHANG);
  if (result == pid_ || (result < 0 && errno == ECHILD)) { pid_ = -1; StopAudio(); }
  return pid_ >= 0;
}
void Process::Stop() {
  if (pid_ >= 0) {
    ...
    if (!result) { kill(pid_, SIGKILL); while (waitpid(pid_, &status, 0) < 0 && errno == EINTR) {} }
    pid_ = -1;
  }
  StopAudio();
}
void Process::StopAudio() {
  ...
  int status = 0; pid_t result = waitpid(audio_pid_, &status, WNOHANG);
  if (!result) kill(audio_pid_, SIGTERM);
  for (int i = 0; !result && i < 25; ++i) {
    std::this_thread::sleep_for(std::chrono::milliseconds(20));
    result = waitpid(audio_pid_, &status, WNOHANG);
  }
  if (!result) { kill(audio_pid_, SIGKILL); while (waitpid(audio_pid_, &status, 0) < 0 && errno == EINTR) {} }
  audio_pid_ = -1;
}
}  // namespace recovery_ui2::media
