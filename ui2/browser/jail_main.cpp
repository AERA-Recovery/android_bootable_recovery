...
int main(int argc, char **argv) {
  ...
  const size_t prefix = recorder ? 14 : media ? 16 : (retroarch || telegram) ? 13 : 14;
  ...
  if (!retroarch && !media && !recorder) WriteResolverConfig(directory);
  ...
  rlimit files{512, 512}, processes{192, 192}, core{0, 0},
  ...
  minijail_change_uid(jail, telegram ? kTelegramUid :
      media ? kMediaUid : recorder ? kRecorderUid : kBrowserUid);
  ...
  execve(program, retroarch ? retroarch_args :
         ...
         telegram ? telegram_environment : browser_environment);
