...
namespace recovery_ui2::retro {
bool Process::Start(const std::string &runtime, int &frame_fd,
                    int &control_fd, std::string &error) {
  frame_fd = control_fd = -1;
  if (pid_ >= 0 || runtime.size() != 19 ||
      runtime.compare(0, 13, "/tmp/aera-ra-")) {
    error = "The verified RetroArch runtime path is invalid.";
    return false;
  }
  struct stat root{};
  if (lstat(runtime.c_str(), &root) || !S_ISDIR(root.st_mode) || root.st_uid ||
      (root.st_mode & 0777) != 0700) {
    error = "The RetroArch runtime is no longer private.";
    return false;
  }
  int frame = memfd_create("aera-retroarch-pixels",
  ...
  if (frame < 0 || ftruncate(frame, kSharedBytes) ||
    ...
    error = "Could not create the private RetroArch display channel.";
    return false;
  }
  ...
  if (child_frame < 0 || child_control < 0) {
    ...
    StopAudio();
    error = "Could not protect RetroArch display descriptors.";
    return false;
  }
